        ...
        <div class="mb-6">
          ...
          <div class="bg-gray-50 p-4 rounded-lg mb-5">
            <div class="flex items-center">
              <div class="h-12 w-12 rounded-lg overflow-hidden bg-gray-200 flex-shrink-0">
                <img :src="opportunity?.image || 'https://via.placeholder.com/48'" alt="" class="h-full w-full object-cover" />
              </div>
              <div class="ml-4">
                <div class="font-medium text-gray-900">{{ opportunity?.titre }}</div>
          ...
          <div class="mb-5">
            <label class="block text-sm font-medium text-gray-700 mb-2">Statut de l'opportunité</label>
            <div class="space-y-3">
              <div 
...
export default {
      ...
      statuses: [
        {
          value: 'en attente',
          label: 'En attente',
          description: 'L\'opportunité est en cours d\'examen',
          bgColor: 'bg-amber-500'
        },
        {
          value: 'actif',
          label: 'Publié',
          description: 'L\'opportunité est visible publiquement',
          bgColor: 'bg-green-500'
        },
        {
          value: 'fermé',
          label: 'Rejeté',
          description: 'L\'opportunité a été refusée',
          bgColor: 'bg-red-500'
        }
      ],
    };
  ...
  watch: {
    opportunity(newVal) {
      if (newVal) {
        this.selectedStatus = newVal.status || 'en attente';
        this.error = '';
      }
    },
    show(newVal) {
  ...
  methods: {
    ...
    async updateStatus() {
      if (!this.selectedStatus || !this.opportunity) return;
      ...
      try {
        ...
        await adminApi.updateOpportuniteStatus(token, this.opportunity.id, this.selectedStatus);
