...
export default {
  ...
  methods: {
    ...
    shareOnTwitter() {
      const url = window.location.href;
      window.location.href = `https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}`;
    }
  },
  async mounted() {
    try {
      const response = await opportuniteApi.getById(this.id);
      if (response.data.opportunite) {
        this.opportunite = response.data.opportunite;
      } else if (response.data.message) {
        this.error = response.data.message; 
      }

      const similarResponse = await opportuniteApi.getSimilarOpportunites(this.id);
      if (similarResponse.data && similarResponse.data.length > 0) {
        this.similarOpportunites = similarResponse.data;
      } else {
        this.error = "Aucune opportunité similaire trouvée.";
      }
