    ...
    <div v-else class="container mx-auto px-4 py-8">
      <div class="max-w-7xl mx-auto">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-100">
            <div class="flex items-center justify-between mb-2">
              <h3 class="text-gray-600 text-sm font-medium">Candidatures</h3>
              <div class="h-10 w-10 rounded-lg bg-purple-100 flex items-center justify-center">
                <i class="fas fa-users text-purple-500"></i>
              </div>
            </div>
            <p class="text-2xl font-bold text-gray-900">{{ stats.postuleStatistics?.totalCandidatures || 0 }}</p>
            <div class="mt-2 text-sm">
              <span class="text-green-500">{{ stats.postuleStatistics?.acceptedCandidatures || 0 }} acceptées</span>,
              <span class="text-amber-500">{{ stats.postuleStatistics?.pendingCandidatures || 0 }} en attente</span>,
              <span class="text-red-500">{{ stats.postuleStatistics?.refusedCandidatures || 0 }} refusées</span>
            </div>
          </div>
          
          <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-100">
            <div class="flex items-center justify-between mb-2">
              <h3 class="text-gray-600 text-sm font-medium">Certificats</h3>
              <div class="h-10 w-10 rounded-lg bg-amber-100 flex items-center justify-center">
                <i class="fas fa-certificate text-amber-500"></i>
              </div>
            </div>
            <p class="text-2xl font-bold text-gray-900">{{ stats.certificationStatistics?.totalCertificationsGiven || 0 }}</p>
            <div class="mt-2 text-sm text-gray-500">
              sur {{ stats.postuleStatistics?.acceptedCandidatures || 0 }} candidatures acceptées
            </div>
          </div>
  ...
  methods: {
    async fetchStats() {
      try {
        ...
        const response = await statistiquesApi.getAssociationStatistics(token);
